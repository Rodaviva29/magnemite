...
-- start.
--
-- An ENUM column rather than a separate type: MariaDB has no CREATE TYPE, and
-- this is what Prisma emits for an enum on this provider.
ALTER TABLE `Rollout`
  ADD COLUMN `hookMode` ENUM('NORMAL', 'POST_ONLY', 'NONE') NOT NULL DEFAULT 'NORMAL';
