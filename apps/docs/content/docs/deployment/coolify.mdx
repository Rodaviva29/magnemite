...
**New Resource → Docker Compose**, pointed at `deploy/compose.coolify.yml`.
...
The admin account is created automatically the moment the hub boots — nothing
to run by hand. Optionally, seed a starter app target, two version sources, a
device group and a first enrollment token:
...
<Callout type="warn">
  Set `ADMIN_EMAIL` **before the first boot** if you want a different one. Changing it later creates a
  *second* admin rather than renaming the first, because the sync upserts on the address.
</Callout>
