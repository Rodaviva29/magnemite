...
- **Scheduler behaviour under load.** 200 devices against
  `MAX_CONCURRENT_JOBS=10` is the only honest way to see the queue drain.
...
- **Stall handling.** A long `MAGNEMITE_FAKE_INSTALL_MS` plus a low
  `JOB_STALL_TIMEOUT` reproduces a re-queue in under a minute.
